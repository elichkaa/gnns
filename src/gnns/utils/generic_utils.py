...
class Timer(object):
    ...
    def interval(self, msg=''):
        ...
        msg_str = '[{} {}] elapsed time: {:.2f}s'.format(
            self.name, msg, elapsed)
        self.start()
        return msg_str

    def finish(self):
        self.total = self.time.time() - self.start_time
        return self.total
...
class DummyLogger(object):
    ...
    def __init__(self, config, dirname, pretrained=False):
