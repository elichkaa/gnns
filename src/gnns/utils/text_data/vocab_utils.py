...
class VocabModel(object):
    def __init__(self, data_set, config):
        ...
        self.word_vocab.build_vocab(
            allWords, vocab_size=config['top_word_vocab'], min_freq=config['min_word_freq'])
        if 'pretrained_word_embed_file' in config:
            self.word_vocab.load_embeddings(
                config['pretrained_word_embed_file'])
        ...
        with open(file_path, 'rb') as f:
            for line in f:
                ...
                if self.embeddings is None:
                    n_dims = len(vec)
                    self.embeddings = np.array(np.random.uniform(
                        low=-scale, high=scale, size=(vocab_size, n_dims)), dtype=dtype)
                    self.embeddings[self.PAD] = np.zeros(n_dims)
                self.embeddings[idx] = vec
                hit_words.add(idx)
        print('Pretrained word embeddings hit ratio: {}'.format(
            len(hit_words) / len(self.index2word)))
