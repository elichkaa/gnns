...
from gnns.idgl.models.graph_clf import GraphClf
from gnns.idgl.models.text_graph import TextGraphRegression, TextGraphClf
from gnns.utils.text_data.vocab_utils import VocabModel
from gnns.utils import constants as Constants
from gnns.utils.generic_utils import to_cuda
from gnns.utils.constants import INF
from gnns.utils.radam import RAdam
...
def batch_decoded_index2word(decoded_tokens, vocab, oov_dict=None):
    if isinstance(decoded_tokens, torch.Tensor):
        decoded_tokens = decoded_tokens.cpu().numpy()
    ...
    for seq in decoded_tokens:
        ...
        for idx in seq:
            ...
            if oov_dict and idx >= len(vocab):
                word = oov_dict.index2word.get(idx, vocab.unk_token)
            else:
...
def eval_batch_output(target_sequences, vocab, oov_dict, *decoded_outputs):
    results = []

    for decoded_tokens in decoded_outputs:
        # Convert decoded tokens to sentences
        if isinstance(decoded_tokens, torch.Tensor):
            predicted_sequences = batch_decoded_index2word(
                decoded_tokens, vocab, oov_dict)
        else:
            predicted_sequences = decoded_tokens

        metrics = {}

        exact_matches = sum(1 for pred, target in zip(predicted_sequences, target_sequences)
                            if pred.strip() == target.strip())
        metrics['exact_match'] = exact_matches / \
            len(target_sequences) if target_sequences else 0.0
        ...
        for pred, target in zip(predicted_sequences, target_sequences):
            ...
            for i, target_token in enumerate(target_tokens):
                if i < len(pred_tokens) and pred_tokens[i] == target_token:
                    correct_tokens += 1

        metrics['token_accuracy'] = correct_tokens / \
            total_tokens if total_tokens > 0 else 0.0

        avg_pred_len = sum(len(pred.split()) for pred in predicted_sequences) / \
            len(predicted_sequences) if predicted_sequences else 0.0
        avg_target_len = sum(len(target.split()) for target in target_sequences) / \
            len(target_sequences) if target_sequences else 0.0
        metrics['length_ratio'] = avg_pred_len / \
            avg_target_len if avg_target_len > 0 else 0.0
