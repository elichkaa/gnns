...
from gnns.idgl.layers.graphlearn import GraphLearner, get_binarized_kneighbors_graph
from gnns.idgl.layers.scalable_graphlearn import AnchorGraphLearner
from gnns.idgl.layers.anchor import AnchorGCN
from gnns.idgl.layers.common import dropout, EncoderRNN
from gnns.idgl.layers.gnn import GCN, GAT
from gnns.utils.generic_utils import normalize_adj, to_cuda, create_mask, batch_normalize_adj
from gnns.utils.constants import VERY_SMALL_NUMBER
...
class TextGraphRegression(nn.Module):
    def __init__(self, config, w_embedding, word_vocab):
        ...
        if not config.get('no_gnn', False):
            ...
            if self.graph_module == 'gcn':
                ...
                raise RuntimeError(
                    'Unknown graph_module: {}'.format(self.graph_module))
            ...
            if self.graph_learn:
                ...
                self.graph_learner2 = graph_learn_fun(hidden_size,
                                                      ...
                                                      topk=config.get(
                                                          'graph_learn_topk2', config['graph_learn_topk']),
                                                      epsilon=config.get(
    ...
    def compute_no_gnn_output(self, context, context_lens):
        ...
        context_vec = self.ctx_rnn_encoder(raw_context_vec, context_lens)[
            1][0].squeeze(0)
        ...
        if self.graph_learn:
            if self.scalable_run:
                ...
                if self.graph_metric_type in ('kernel', 'weighted_cosine'):
                    ...
                    adj = raw_adj / \
                        torch.clamp(torch.sum(raw_adj, dim=-1,
                ...
                if graph_skip_conn in (0, None):
                    if graph_include_self:
                        adj = adj + \
                            to_cuda(torch.eye(adj.size(0)), self.device)
                else:
                    adj = graph_skip_conn * init_adj + \
                        (1 - graph_skip_conn) * adj
    ...
    def compute_output(self, node_vec, node_mask=None):
        graph_vec = self.graph_maxpool(
            node_vec.transpose(-1, -2), node_mask=node_mask)
        output = self.linear_out(graph_vec).squeeze(-1)
        return torch.sigmoid(output)

    def prepare_init_graph(self, context, context_lens):
        context_mask = create_mask(
            context_lens, context.size(-1), device=self.device)
        ...
        init_adj = self.compute_init_adj(raw_context_vec.detach(
        ), self.config['input_graph_knn_size'], mask=context_mask)
    ...
    def graph_maxpool(self, node_vec, node_mask=None):
        ...
        graph_embedding = F.max_pool1d(
            node_vec, kernel_size=node_vec.size(-1)).squeeze(-1)
        return graph_embedding

    def compute_init_adj(self, features, knn_size, mask=None):
        adj = get_binarized_kneighbors_graph(
            features, knn_size, mask=mask, device=self.device)
