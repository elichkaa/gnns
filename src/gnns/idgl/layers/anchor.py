...
def batch_sample_anchors(node_vec, ratio, node_mask=None, device=None):
    ...
    anchor_vec = batch_select_from_tensor(
        node_vec, idx, max_num_anchors, device)
...
def batch_select_from_tensor(node_vec, idx, max_num_anchors, device=None):
    ...
    for i in range(node_vec.size(0)):
        ...
        if len(tmp_anchor_vec) < max_num_anchors:
            dummy_anchor_vec = to_cuda(torch.zeros(
                (max_num_anchors - len(tmp_anchor_vec), node_vec.size(-1))), device)
            tmp_anchor_vec = torch.cat(
                [tmp_anchor_vec, dummy_anchor_vec], dim=-2)
...
def compute_anchor_adj(node_anchor_adj, anchor_mask=None):
    ...
    anchor_norm = torch.clamp(anchor_node_adj.sum(
    ...
    anchor_adj = torch.matmul(
        anchor_node_adj, anchor_norm.unsqueeze(-1) * node_anchor_adj)

    markoff_value = 0
    if anchor_mask is not None:
        anchor_adj = anchor_adj.masked_fill_(
            1 - anchor_mask.byte().unsqueeze(-1), markoff_value)
        anchor_adj = anchor_adj.masked_fill_(
...
class AnchorGCNLayer(nn.Module):
    ...
    def forward(self, input, adj, anchor_mp=True, batch_norm=True):
        ...
        if anchor_mp:
            node_anchor_adj = adj
            node_norm = node_anchor_adj / \
                torch.clamp(torch.sum(node_anchor_adj, dim=-2,
                            keepdim=True), min=VERY_SMALL_NUMBER)
            anchor_norm = node_anchor_adj / \
                torch.clamp(torch.sum(node_anchor_adj, dim=-1,
                            keepdim=True), min=VERY_SMALL_NUMBER)
            output = torch.matmul(anchor_norm, torch.matmul(
...
class AnchorGCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, graph_hops, dropout, batch_norm=False):
        ...
        self.graph_encoders.append(AnchorGCNLayer(
        ...
        self.graph_encoders.append(
            AnchorGCNLayer(nhid, nclass, batch_norm=False))
