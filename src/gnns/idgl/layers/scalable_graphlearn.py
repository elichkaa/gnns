...
class AnchorGraphLearner(nn.Module):
    def __init__(self, input_size, hidden_size, topk=None, epsilon=None, num_pers=16, metric_type='attention', device=None):
        ...
        if metric_type == 'attention':
            self.linear_sims = nn.ModuleList(
                [nn.Linear(input_size, hidden_size, bias=False) for _ in range(num_pers)])
            print(
                '[ Multi-perspective {} AnchorGraphLearner: {} ]'.format(metric_type, num_pers))
            ...
            self.linear_sims1 = nn.ModuleList(
                [nn.Linear(input_size, 1, bias=False) for _ in range(num_pers)])
    ...
    def forward(self, context, anchors, ctx_mask=None, anchor_mask=None):
        ...
        if self.metric_type == 'attention':
            ...
            for _ in range(len(self.linear_sims)):
                ...
                attention += torch.matmul(context_fc,
                                          context_fc.transpose(-1, -2))
            ...
            attention = torch.matmul(
                context_norm, anchors_norm.transpose(-1, -2)).mean(0)
            markoff_value = 0

        elif self.metric_type == 'transformer':
            Q = self.linear_sim1(context)
            attention = torch.matmul(
                Q, Q.transpose(-1, -2)) / math.sqrt(Q.shape[-1])
            markoff_value = -INF
            ...
            for _ in range(len(self.linear_sims1)):
                ...
                attention.append(self.leakyrelu(
                    a_input1 + a_input2.transpose(-1, -2)))
            ...
            attention = torch.exp(-0.5 * attention *
                                  (self.precision_inv_dis**2))

            markoff_value = 0

        elif self.metric_type == 'cosine':
            context_norm = context.div(torch.norm(
                context, p=2, dim=-1, keepdim=True))
            attention = torch.mm(
                context_norm, context_norm.transpose(-1, -2)).detach()
            markoff_value = 0

        if ctx_mask is not None:
            attention = attention.masked_fill_(
                1 - ctx_mask.byte().unsqueeze(-1), markoff_value)

        if anchor_mask is not None:
            attention = attention.masked_fill_(
                1 - anchor_mask.byte().unsqueeze(-2), markoff_value)
        ...
        if self.topk is not None:
            attention = self.build_knn_neighbourhood(
                attention, self.topk, markoff_value)
    ...
    def build_knn_neighbourhood(self, attention, topk, markoff_value):
        ...
        weighted_adjacency_matrix = to_cuda(
            (markoff_value * torch.ones_like(attention)).scatter_(-1, knn_ind, knn_val), self.device)
    ...
    def compute_distance_mat(self, X, weight=None):
        ...
        dists = -2 * torch.matmul(trans_X, X.transpose(-1, -2)) + \
            norm.unsqueeze(0) + norm.unsqueeze(1)
        return dists
...
def get_binarized_kneighbors_graph(features, topk, mask=None, device=None):
    ...
    features_norm = features.div(torch.norm(
        features, p=2, dim=-1, keepdim=True))
